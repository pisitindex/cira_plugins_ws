...
void LicensePlateModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void LicensePlateModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void LicensePlateModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &LicensePlateModel::runProcess, _flowDataIn);
