...
void undistortModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void undistortModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void undistortModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &undistortModel::runProcess, _flowDataIn);
