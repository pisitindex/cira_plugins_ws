...
void affine_aiModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void affine_aiModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void affine_aiModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &affine_aiModel::runProcess, _flowDataIn);
