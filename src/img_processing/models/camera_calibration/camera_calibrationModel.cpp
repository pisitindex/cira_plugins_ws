#include "camera_calibrationModel.hpp"
#include <cira_lib_bernoulli/datatype/FlowData.hpp>

#include <QtConcurrent/QtConcurrent>

camera_calibrationModel::
camera_calibrationModel()
{
  if(!GlobalData::isInitialized) return;

  form = new Formcamera_calibration();
...
void camera_calibrationModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void camera_calibrationModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void camera_calibrationModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &camera_calibrationModel::runProcess, _flowDataIn);
